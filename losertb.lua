...
RunService.RenderStepped:Connect(function()
    ...
    if (_G.Settings.Modes.Tryouts.Enabled and hostshot) or _G.Settings.Modes.Generic.Enabled then
        local target = mouse.Target
        local parent = target.Parent
        local charr = target:FindFirstAncestorWhichIsA("Model")
        local grandparentName = tostring(charr)
        local player5 = Players:GetPlayerFromCharacter(charr)
        
        if player5 then
            if charr.BodyEffects["K.O"].Value == true then
                print("OK IT PASSED")
